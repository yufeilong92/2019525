 ...
 */
public class UpDeleteErrorSqlteHelp {
    private Context mContext;
    private static volatile UpDeleteErrorSqlteHelp _instance = null;
    private final DbQueryUtil mDbQueryUtil;
    private final SQLiteDatabase mSqLiteDatabase;

    private UpDeleteErrorSqlteHelp(Context context) {
        this.mContext = context;
        mSqLiteDatabase = createtable();
        mDbQueryUtil = DbQueryUtil.get_Instance();
    }

    public static UpDeleteErrorSqlteHelp getInstance(Context context) {
        if (_instance == null) {
            synchronized (UpDeleteErrorSqlteHelp.class) {
                if (_instance == null) {
                    _instance = new UpDeleteErrorSqlteHelp(context);
                }
            }
        }
        return _instance;
    ...
    public void addErrorItem(ErrorSqliteVo sqliteVo) {
        ...
        if (vo == null||vo.getQuesitonid()==0) {
            ...
            mSqLiteDatabase.insert(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR, null, values);
        } else {
            ContentValues values = getContentValues(sqliteVo);
            mSqLiteDatabase.update(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR, values,
    ...
    public int queryCountWithCourseid(int coursedid) {
        if (empty()) return 0;
        String sql = "select count(*) from " + DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR + "  where courseid=" + coursedid;
    ...
    public List<ErrorSqliteVo> getErrorLists(int courseid) {
        ...
        Cursor query = mSqLiteDatabase.query(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR, null,
      ...
      public List<ErrorSqliteVo> queryAllList(){
        ...
        Cursor query = mSqLiteDatabase.query(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR, null, null, null, null, null
                , "time desc","500");
    ...
    public void delectErrorItem(int id) {
        if (empty()) return;
        mSqLiteDatabase.delete(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR, "id=?",
                new String[]{String.valueOf(id)});
    }
    //删除某条数据
    public void delectError() {
        if (empty()) return;
        mSqLiteDatabase.delete(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR, null,null);
    }
    ...
    public void deleteErrorItem(int courseid, int chapterid, int questionid) {
        if (empty()) return ;
        mSqLiteDatabase.delete(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR,"courseid=? and chapterid=? and quesitonid=?",
     ...
     */
    public void deleteErrorItem(ErrorSqliteVo vo) {
        if (empty()) return ;
        mSqLiteDatabase.delete(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR,"courseid=? and chapterid=? and quesitonid=?",
                new String[]{String.valueOf(vo.getCourseid()), String.valueOf(vo.getChapterid()),
                        String.valueOf(vo.getQuesitonid())});
    }
    ...
    public void upDataRightNumber(int id, int rihgtnumber) {
        ...
        mSqLiteDatabase.update(DataMessageVo.USER_INFOM_TABLE_DETUP_ERROR, values, "id=?",
                new String[]{String.valueOf(id)});
    }
