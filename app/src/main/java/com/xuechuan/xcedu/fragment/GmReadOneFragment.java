...
public class GmReadOneFragment extends BaseFragment implements TestObserver, View.OnClickListener, GmEvalueContract.View {
    ...
    private static String TAG = "【" + GmReadOneFragment.class + "】==";
    private View view;
...
//    private AnswerEvaluateAdapter mEvalueAdapter;
     ...
     * 模拟考试
     */
    private MockExamActivity mMockExamActivity;
    //    private WeiNestedScrollView mSmsvOneLayout;
    private SmartScrollView mSmsvOneLayout;
     ...
     * @return
     */
    public static GmReadOneFragment newInstance(QuestionCaseVo vo, int postion, String mCouserid) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            mQuestionCaseVo = (QuestionCaseVo) getArguments().getSerializable(ARG_PARAM_DATA);
...
*/
    ...
    private void initActivity() {
        //初始化选项图片
        if (getActivity() instanceof MockExamActivity) {//模拟考试
            mMockExamActivity = (MockExamActivity) getActivity();
            mQuestionSqliteVo = mMockExamActivity.getQuestionVo(mQuestionCaseVo.getId());
        }
...
//        mEvalueAdapter = new AnswerEvaluateAdapter(mContext, mArray);
   ...
   /*     mSmsvOneLayout.setmSmartScrollChangedListener(new WeiNestedScrollView.ISmartScrollChangedListener() {
        ...
        mSmsvOneLayout.setScanScrollChangedListener(new SmartScrollView.ISmartScrollChangedListener() {
            @Override
            public void onScrolledToBottom() {
                if (!doOpenNewWork()) {
                    return;
...
//        mSmsvOneLayout = (WeiNestedScrollView) view.findViewById(R.id.smsv_one_layout);
        mSmsvOneLayout = (SmartScrollView) view.findViewById(R.id.smsv_one_layout);
    ...
    public void refresh() {
        ...
        setImgBg();
        setFontSize();
        if (mMockExamActivity != null) {
            mMockExamActivity.changerColor(mGmReadColorManger);
            if (mMockExamActivity.getSubmitAble()) {
                mMockExamActivity.changerCollect();
            }
        }
        initDoEvent();
    }
    ...
    private void initDoEvent() {
        ...
        if (vo.getQuestiontype() == 2) {//单选
            ...
            setSelectABCDEClick(!mMockExamActivity.getSubmitAble());
            doRadioEventView();
        } else if (vo.getQuestiontype() == 3) {//多选
            if (vo.getSelectA() == 1) {
...
//                mBtnGmoneSuerAnswer.setVisibility(mMockExamActivity.getSubmitAble() ? View.GONE : View.VISIBLE);
    ...
    private void setColor() {
        ...
        mGmOneRoolLayout.setBackgroundColor(mGmReadColorManger.getmLayoutBgColor());
        //选择布局
...
//        mBtnGmoneSuerAnswer.setVisibility(isShow ? View.VISIBLE : View.GONE);
    ...
    private void registerOberovce() {
        ...
        mSubsciber = new GmReadOneFragment().subsciber();
...
//        setSelectABCDEClick(false);
    ...
    public void showGmSetting() {
        ...
        mPopSetting.showAtLocation(mGmOneRoolLayout, Gravity.BOTTOM, 0, 0);
    ...
    private void doSelectMoreEvent() {
        if (!mMockExamActivity.getSubmitAble()) {//用户未交卷
    ...
    private void doRadioEventView() {
        //用户不提交不显示结果
        if (!mMockExamActivity.getSubmitAble()) return;
        //判断用户是否购买
    ...
    private void doSaveEvent(DoBankSqliteVo vo) {
        if (mMockExamActivity != null) {
            mMockExamActivity.saveUserDoLog(vo);
        }
...
//                        hideScreenshot(false);
                        String pic = ScreenShot.savePic(ScreenShot.getBitmapByView(mContext, mSmsvOneLayout));
                        //初始化选项图片
                        if (getActivity() instanceof MockExamActivity) {//模拟考试
                            mMockExamActivity = (MockExamActivity) getActivity();
                ...
                qqzon.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (getActivity() instanceof MockExamActivity) {//模拟考试
                            mMockExamActivity = (MockExamActivity) getActivity();
//                            ShareUtils.shareImg(mMockExamActivity, mQuestionSqliteVo.getQuestionStr(),
                            ShareUtils.shareImg(mMockExamActivity, mQuestionSqliteVo.getQuestionString(),
                                    pic, SHARE_MEDIA.QQ);
                        }
                        EventBus.getDefault().postSticky(new GmChangerColorEvent(0,5,false));
...
//                            ShareUtils.shareImg(mMockExamActivity, mQuestionSqliteVo.getQuestionStr(),
        ...
        mPopShare.showAtLocation(mGmOneRoolLayout, Gravity.BOTTOM, 0, 0);
