...
import com.xuechuan.xcedu.ui.bank.newBankActivity.SeachResultActivity;
...
public class GmReadSeachFragment extends BaseFragment implements TestObserver, GmEvalueContract.View, View.OnClickListener {
    ...
    private static String TAG = "【" + GmReadSeachFragment.class + "】==";
    private View view;
...
//    private AnswerEvaluateAdapter mEvalueAdapter;
     ...
     * 搜索
     */
    private SeachResultActivity mSearchResultActivity;
     ...
     * @return
     */
    public static GmReadSeachFragment newInstance(CaseCardVo vo, int postion, String mCouserid) {
        GmReadSeachFragment fragment = new GmReadSeachFragment();
...
*/
    ...
    protected void initCreateView(View view, Bundle savedInstanceState) {
        ...
        if (mCaseCardVo == null) {
            ...
            if (mSearchResultActivity != null) {
                mQuestionSqliteVo = mSearchResultActivity.getQuestionVo(mCaseCardVo.getCasevo().getId());
            }
        }
        ...
        //解读数据
        readDataBindView();
        if (!mSearchResultActivity.getHaveChilds()) {
            //初始化适配器
            initEvalueAdapter();
        }
        //注册观察者
    ...
    private void initActivity() {
        //初始化选项图片
        if (getActivity() instanceof SeachResultActivity) {//章节练习
            mSearchResultActivity = (SeachResultActivity) getActivity();
        }
...
// TODO: 2019.01.18  处理索搜
        boolean haveChilds = false;
        if (mSearchResultActivity != null) {
            haveChilds = mSearchResultActivity.getHaveChilds();
        }
        if (!haveChilds) {//是否阅读题
            //阅读理解头
    ...
    public void refresh() {
        ...
        setFontSize();
        if (mSearchResultActivity != null) {
            mSearchResultActivity.changerColor(mGmReadColorManger);
            mSearchResultActivity.changerCllect();
        }
    ...
    private void initDoEvent() {
        ...
        if (mSearchResultActivity != null) {
            vo = mSearchResultActivity.queryUserData(mQuestionSqliteVo.getQuestion_id());
        }
    ...
    private void initTitleRed() {
        ...
        Spanned spanned = StringUtil.repaceExamStr(mQuestionSqliteVo.getKeywordsString(),
...
//        mTvGmZoneQuestionTitle.setText(getString(question));
        ...
        //简答新解析
//        mTvGmEightOneAnswerTitleContent.setText(mQuestionSqliteVo.getExplainedStr());
        ...
        if (mSearchResultActivity != null) {
            ...
            mTvGmTitleAllnumber.setText(String.valueOf(mSearchResultActivity.getFuPostionList()));
        }
    ...
    private String doMainEvent(String question) {
        final int width = mIvGmZoneQuestintype.getLayoutParams().width+20;
        if (mSearchResultActivity != null) {
            if (!mSearchResultActivity.getMainQuesiton()) {
//                String empty = "               ";
                cound = 20;
        ...
        if (mSearchResultActivity != null) {
            ...
            if (mSearchResultActivity.getHaveChilds()) {
                if (mSearchResultActivity.getMainQuesiton()) {
                    ...
                    mBtnGmsevenAnswer.setVisibility(View.INVISIBLE);
                }
...
//        String keywordStr = mQuestionSqliteVo.getKeywordStr();
    ...
    private void registerOberovce() {
        ...
        mSubsciber = new GmReadSeachFragment().subsciber();
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_gmseven_answer://答题按钮
                if (mSearchResultActivity != null) {
                    if (mCaseCardVo.getList() != null && !mCaseCardVo.getList().isEmpty()) {
                        mSearchResultActivity.doEventHine(true);
                        mSearchResultActivity.changerSmallQuestion(mPostion);
                    } else {
            ...
            case R.id.iv_gmTitle_mulu://目录
                if (mSearchResultActivity != null) {
                    mSearchResultActivity.changerMainQuesiton();
                    mSearchResultActivity.doEventHine(false);
                }
            ...
            case R.id.ll_gm_three_note_bg://笔记
                Intent intent = NoteMakeActivity.start_Intent(mContext, mQuestionSqliteVo.getQuestion_id(),
                        ...
                        "", mQuestionSqliteVo.getKeywordsString(), mQuestionSqliteVo.getKeywordsString(), 1);
                mContext.startActivity(intent);
                break;
            case R.id.iv_gm_eight_one_yuyin_img://语音
                if (mSearchResultActivity != null) {
                    mSearchResultActivity.videoInput(mEtEightOneInputContent);
                }
                break;
    ...
    private void doNextGo(boolean b) {
        ...
        if (!b) return;
        //初始化选项图片
        if (mSearchResultActivity != null) {//章节练习
            mSearchResultActivity.doRightGo(mPostion);
        }
    ...
    private void doOpenNewWorkEvent() {
        boolean openNetwork = PolyvSDKUtil.isOpenNetwork(mContext);
        if (!openNetwork) {
            mLlFourEvaluateLayout.setVisibility(View.GONE);
            mLlGmthreeNoteLayout.setVisibility(View.GONE);
        }
    ...
    private void doSaveEvent(DoBankSqliteVo vo, ErrorSqliteVo mErrorVo) {
        if (mSearchResultActivity != null) {
            mSearchResultActivity.saveUserDoLog(vo);
            mSearchResultActivity.doErrorLog(mErrorVo);
        }
...
//                        hideScreenshot(false);
                        ...
                        if (getActivity() instanceof SeachResultActivity) {//章节练习
                            mSearchResultActivity = (SeachResultActivity) getActivity();
                            ShareUtils.shareImg(mSearchResultActivity, mQuestionSqliteVo.getKeywordsString(),
                                    pic, SHARE_MEDIA.QQ);
                        }
